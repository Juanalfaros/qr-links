...
- Personalización de apariencia (panel de superadmin, y opcionalmente en `/setup`): selector de color primario y de acento (con overrides individuales para cada uno de los 5 colores secundarios), redondeo de esquinas, estilo del sidebar y color de QR por defecto, con preview en vivo en claro y oscuro — todo el sistema de diseño es OKLCH, así que solo se usa el matiz de cada color elegido y el contraste queda garantizado en ambos temas sin importar qué color se pique.
